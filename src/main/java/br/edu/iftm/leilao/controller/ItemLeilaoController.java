...
public class ItemLeilaoController {
	...
	@PostMapping("/{id}")
	public Lance registraLance(@RequestBody Lance lance, @PathVariable Long id) {
		return itemdeLeilaoService.registraLance(lance, id);

	}

	@PatchMapping("/{id}")
	public Lance atualiza(@PathVariable Long id) {
		return itemdeLeilaoService.atualiza(id);
	}
